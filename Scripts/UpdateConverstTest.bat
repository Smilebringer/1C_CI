...
SET "repo_dev_dest=\\ru-s-mosc-da03\_ConfStorage\ERP2"
...
::deployka session lock -rac %racpath% -ras %server%:1545 -db %db_name% -db-user %db_user% -db-pwd %db_pwd% -lockmessage "%lockmessage%" -lockstartat %time_for_user_to_finish_job% -lockuccode %lock_code%
call deployka session lock -rac %racpath% -ras %server%:1545 -db %db_name% -db-user %db_user% -db-pwd %db_pwd% -lockmessage "%lockmessage%" -lockuccode %lock_code% -lockstartat %time_for_user_to_finish_job% -v8version %v8version%
echo.
...
call deployka session unlock -rac %racpath% -ras %server%:1545 -db %db_name% -db-user %db_user% -db-pwd %db_pwd% -v8version %v8version%
 ...
 ::Обновим 2 тестовые бухни
SET "repo_dev_dest=\\ru-s-mosc-da03\_ConfStorage\BUH3"
 
SET db_name=buh3_alo_test_convert3
SET repo_dev_user=Администратор_%db_name%
SET "ConnectionString=/IBConnectionString""Srvr=%server%;Ref=%db_name%;"""
...
SET db_name=buh3_log_test_convert3
SET repo_dev_user=Администратор_%db_name%
SET "ConnectionString=/IBConnectionString""Srvr=%server%;Ref=%db_name%;"""
